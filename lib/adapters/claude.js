 ...
 *
 * Claude Code CLI adapter. Uses --print + stream-json for non-interactive
 * one-shot execution with structured output.
 *
...
export const claudeAdapter = {
  ...
  buildSpawnConfig(prompt, _cwd, opts = {}) {
    ...
    args.push(prompt);
    return { bin: "claude", args, stdinMode: null };
  },
    ...
    switch (event.type) {
      case "system":
        if (event.subtype === "init" && event.session_id) {
          return { type: "session_init", data: { sessionId: event.session_id } };
        }
      ...
      case "assistant": {
        ...
        for (const block of content) {
          ...
          if (block.type === "tool_use") {
            ...
            const isFileChange = ["Write", "Edit", "create_file", "edit_file"].includes(name);
            if (isFileChange) {
              return {
                type: "file_change",
                data: {
                  path: input.file_path || input.path || "",
                  action: name.toLowerCase().includes("write") ? "write" : "edit",
                  tool: name,
                },
              };
            }
      ...
      case "result": {
        ...
        if (event.is_error || subtype !== "success") {
          ...
          const message = errors.filter((e) => typeof e === "string").join(" | ")
            || event.result || "Execution failed";
          return { type: "error", data: { message, subtype } };
        }
        return { type: "result", data: { text: event.result || "", sessionId: event.session_id } };
      }
  ...
  getSummary(steps) {
    ...
    return {
      text: resultStep?.data?.text || textSteps.map((s) => s.data?.text || "").join("\n"),
...
function summarizeToolCall(name, input) {
  ...
  if (name === "Write" || name === "write_file") return `Writing ${input.file_path || input.path || "file"}`;
  if (name === "Edit" || name === "edit_file") return `Editing ${input.file_path || input.path || "file"}`;
  if (name === "Bash" || name === "shell") return `Running: ${(input.command || "").slice(0, 80)}`;
  if (name === "Glob") return `Searching: ${input.pattern || ""}`;
  if (name === "Grep") return `Grepping: ${input.pattern || ""}`;
  if (name === "mcp__chat__web_search") return `Web search: ${input.query || ""}`;
  return name;
}
