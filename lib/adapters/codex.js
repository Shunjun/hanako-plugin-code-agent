...
export const codexAdapter = {
  ...
  buildSpawnConfig(_prompt, _cwd, opts = {}) {
    ...
    // Confirmation MCP server setup via -c flags (slock-daemon pattern)
    ...
    args.push(
      "-c",
      `mcp_servers.code-agent-confirmation.command="node"`,
      "-c",
      `mcp_servers.code-agent-confirmation.args='${JSON.stringify([mcpServerPath])}'`,
      "-c",
      `mcp_servers.code-agent-confirmation.env.CODE_AGENT_SOCKET_PATH="${socketPath}"`,
      "-c",
      "mcp_servers.code-agent-confirmation.enabled=true",
    );

    if (Array.isArray(opts.extraArgs)) {
      args.push(...opts.extraArgs);
    }
    const bin = findBinary("codex") || "codex";
    return { bin, args, stdinMode: "jsonrpc", socketPath };
  },

  /**
   * Clean up temporary resources (no-op for Codex, MCP server dies with process).
   */
  cleanupSpawnConfig(_spawnConfig) {
    // Codex MCP servers are child processes of the CLI — they die with it.
  ...
  initStdin(proc, opts = {}) {
    ...
    // Merge confirmation prompt with user-provided system prompt
      ...
      case "item/started": {
        ...
        switch (item.type) {
          ...
          case "mcpToolCall": {
            ...
            if (item.tool === "request_confirmation") {
              return {
                type: "confirmation_request",
                data: {
                  toolUseId: item.id || "",
                  question: item.arguments?.question || "Confirm?",
                  level: item.arguments?.level || "user",
                  tool: toolName,
                },
              };
            }
