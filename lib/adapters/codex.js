  ...
  parseLine(line) {
    ...
    switch (method) {
      ...
      case "item/agentMessage/delta":
        return { type: "text", data: { text: params.delta || "" }, streaming: true };

      case "item/reasoning/summaryTextDelta":
      case "item/reasoning/textDelta":
        return { type: "thinking", data: { text: params.delta || "" }, streaming: true };
      ...
      case "item/started": {
        ...
        switch (item.type) {
          ...
          case "mcpToolCall": {
            const toolName = item.server === "chat" ? `mcp__chat__${item.tool}` : `${item.server}_${item.tool}`;
            return {
              type: "tool_call",
              data: { tool: toolName, summary: toolName, input: item.arguments },
            };
          }
          case "webSearch":
            return {
