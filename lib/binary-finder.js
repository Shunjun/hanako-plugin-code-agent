...
function claudeCandidates() {
  ...
  if (isMac || isLinux) {
    ...
    bins.push(join(home, ".local/bin/claude"));
  }
  // System-wide
...
function codexCandidates() {
  ...
  if (isMac || isLinux) {
    ...
    bins.push(join(home, ".local/bin/codex"));
  }
...
/**
 * Expand a glob-like pattern: /foo/*\/bar to list of /foo/<each>/bar
 */
function globJoin(base, middle, suffix) {
  const parentDir = resolve(join(base, middle));
  try {
    if (!existsSync(parentDir)) return [];
    const entries = readdirSync(parentDir, { withFileTypes: true });
    const suffixAfterStar = suffix.slice(suffix.indexOf("*") + 1);
    return entries
      .filter((e) => e.isDirectory() && !e.name.startsWith("."))
      .map((e) => join(parentDir, e.name, suffixAfterStar));
  } catch {
    return [];
  }
}
