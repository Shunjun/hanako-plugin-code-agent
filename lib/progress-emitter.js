...
export class ProgressEmitter {
  ...
  emitConfirmationRequired(taskId, confirmation) {
    // Reset deferred state and notify parent agent
    this.bus
      .request("deferred:retry", {
        taskId,
      ...
      .then(() => {
        const parts = [
          `[Code Agent] Confirmation required (${taskId})`,
          `confirmationId: ${confirmation.confirmationId}`,
          `level: ${confirmation.level}`,
          `question: ${confirmation.question}`,
        ];
        return this.bus.request("deferred:resolve", {
          taskId,
          result: { text: parts.join("\n") },
        });
      })
      .catch((err) =>
        console.error(`[code-agent] emitConfirmationRequired failed:`, err),
      );
  }
