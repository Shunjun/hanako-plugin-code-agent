...
export class TaskStore {
  ...
  addConfirmation(taskId, confirmation) {
    const task = this._tasks.get(taskId);
    if (!task) return;
    if (!Array.isArray(task.confirmations)) task.confirmations = [];
    task.confirmations.push(confirmation);
    this._scheduleFlush();
  }

  updateConfirmation(taskId, confirmationId, patch) {
    const task = this._tasks.get(taskId);
    if (!task || !Array.isArray(task.confirmations)) return null;
    const conf = task.confirmations.find((c) => c.confirmationId === confirmationId);
    if (!conf) return null;
